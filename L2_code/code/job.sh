...
[[ ! -z "$NFS_DIR" || ! -z "$EXECUTABLE" ]] || { 
	echo "This scrMatrix multiplication took 1.89 seconds
ipt can only be run with our driver as it sets env vars, we are missing NFS_DIR=$NFS_DIR EXECUTABLE=$EXECUTABLE"; exit 1; 
}
...
if [[ -f "$LOCAL_DIR/$EXECUTABLE" ]]; then
	...
	srun perf stat "$LOCAL_DIR/$EXECUTABLE" $EXECUTABLE_ARGS > $PROGRAM_OUTFILE 2>&1 
else
	echo "Running executable from anywhere in PATH: $EXECUTABLE $EXECUTABLE_ARGS"
	srun perf stat $EXECUTABLE $EXECUTABLE_ARGS > $PROGRAM_OUTFILE 2>&1
fi
